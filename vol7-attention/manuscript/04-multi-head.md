...
つまり Transformer は式(1)を**1回だけでは使いません**。同じ計算を $h$ 個並べて走らせ、結果を束ねます。図1のアーキテクチャ図の橙色ブロックの正体は、式(1)そのものではなくこの「束」の方でした。
...
手順は全部で4つです。(1) 線形射影を $h$ 通り用意し、(2) それぞれの射影先で attention を並列に実行し、(3) 出力を連結して、(4) もう一度射影します。線形射影は第1巻5章、attention は前章の装備ですから、読めない単語はありません。読めないのは**動機**です。論文は次の2文で答えます。この章でいちばん重要な2文です。
...
"averaging inhibits this"——平均化が「これ」を妨げます。「これ」とは直前の "jointly attend"、**複数の見方を同時に持つこと**です。なぜ平均化がそれを妨げるのでしょうか。

前章で確かめたとおり、attention の出力は **value の重み付き平均**です。重みは softmax の出力なので非負で足すと1です。つまり1回の attention が query 1本に作れるのは、「どこをどれだけ見るか」という**ただ1つの配分表**と、その配分による**ただ1つの平均**だけです。
...
value を2次元で $\mathbf{v}_{太郎} = (1, 0)$、$\mathbf{v}_{駅} = (0, 1)$ とします(他は省略)。「主語の見方」は配分 $(1, 0)$ で出力 $(1, 0)$、「場所の見方」は配分 $(0, 1)$ で出力 $(0, 1)$ です。

1回の attention では配分表を1つしか持てず、折衷して $(0.5,\ 0.5)$ とするしかありません。出力は

$$0.5 \cdot (1, 0) + 0.5 \cdot (0, 1) = (0.5,\ 0.5)$$

この $(0.5, 0.5)$ が困りものです。後段の層には「主語は太郎、場所は駅」という2つの答えが**足し合わされて1つに溶けた**ものしか届きません。「value が $(0.5, 0.5)$ である別の単語を1点だけ見た」場合と出力上は区別がつきません。2つの見方は平均を取った瞬間に互いを薄め合う——これが "averaging inhibits this" です。

head が2つあれば話は変わります。head 1 は配分 $(1, 0)$ で $(1, 0)$、head 2 は配分 $(0, 1)$ で $(0, 1)$。手順(3)で2出力を**連結**すれば $(1, 0, 0, 1)$。平均ではなく**並置**なので、両方の答えが別の成分として残ります。どう混ぜるかは最後の射影(手順4)が学習で決めればよいのです。

「異なる表現部分空間(representation subspaces)」にも触れておきます。head ごとの射影は、$d_{model} = 512$ 次元から $d_k = 64$ 次元を切り出す線形変換(第1巻5章)——512次元のうち**どの側面を残すかを選ぶ変換**です。射影が違えば内積、つまり類似度が変わります。「文法的な働きが似ている」と「意味が似ている」は別の「似ている」であり、別の射影がそれぞれを拾います。head とは、**類似度の測り方そのものを複数持つ**仕掛けでもあるのです。
...
その予告を回収します。**$i$ は「何番目の見方か」、つまり head の番号です。** 第1巻5.5の「役割への変換」一式($W_i^Q, W_i^K, W_i^V$ の3枚)を、論文は $h$ 組用意します。$i$ 組目の射影で作った $Q, K, V$ に式(1)を適用したものが $\mathrm{head}_i$ です。役割(query / key / value)× 視点(head $1 \ldots h$)で、射影行列は計 $3h$ 枚です。第1巻5.5の読みは「同じ入力を、**視点ごとに別々の**役割に変換する」へと完成します。$W^Q$ が `(512, 512)` ではなく `(512, 64)` だったのは1 head 分の射影だったから——これが6巻越しの種明かしです。

記号の罠を先回りします。この式の $Q, K, V$ は**射影前の入力**で、$QW_i^Q$ と掛けて初めて式(1)に渡ります。射影**済み**の行列を指していた式(1)の $Q, K, V$ とは立場が違う——同じ文字の二重使用です。self-attention なら3つとも同じ行列 $X$ です(配線は次章 3.2.3 の主題)。混同を避けるため、実装では射影前の入力を `X_q`(query の出どころ)、`X_kv`(key と value の出どころ)と呼び分けます。
...
$d_k = d_{model} / h$ です。512次元を8等分して64次元ずつ——「$d_{model}$ を $h$ 個の head に**分割**する」と呼ばれる理由です。入力を $X$ `(n, 512)` として、式全体を shape で読み通します。
...
入口が `(n, 512)`、出口も `(n, 512)` です。序章0.3で約束した sub-layer の規格——同じ shape を受け取り同じ shape を返す、residual を足すための設計(第2章)——を、multi-head attention はこの連結と出力射影で満たしています。

$W^O$ の役割も言葉にします。連結しただけの `(n, 512)` は、head 1 の答えが第1〜64列、head 2 が第65〜128列……と**縦割りのまま並んでいるだけ**です。4.1の例でいえば「主語の答え」と「場所の答え」が別区画に置いてある状態です。$W^O$ はこの512列を混ぜ合わせて1つの表現に編集する全結合(第1巻6章)で、どう組み合わせるかは学習に委ねられます。

## 4.3 テンソル整形の実務 — (batch, seq, d_model) → (batch, h, seq, d_k) の reshape / transpose(第1巻6.4「行列の束」がついに本番)

式は読めました。次は実装の段取りです。素朴に書くなら $W_1^Q$ から $W_8^Q$ まで `(512, 64)` を8枚持ち、forループで head を1本ずつ計算します。動きますが、第1巻からの問いをここでも立てます。**このループ、行列演算にまとめられないでしょうか。** まとめられます。鍵は射影の側と attention の側に1つずつ、計2つです。

なお、見出しの batch 軸は説明の間は省きます。文1本($X$ `(n, 512)`)で進め、結論を先に言えばバッチは**先頭に軸が1本増えるだけ**で、コードは1文字も変わりません。

**鍵その1: 8枚の射影は1枚の大きな射影にまとめられます。** `(512, 64)` を8枚、横に貼り合わせます。

$$W^Q = \big[\, W_1^Q \;\big|\; W_2^Q \;\big|\; \cdots \;\big|\; W_8^Q \,\big] \qquad \texttt{(512, 64)} \times 8 \to \texttt{(512, 512)}$$

この $W^Q$ で一度に射影すると $XW^Q$ は `(n, 512) @ (512, 512) → (n, 512)` です。行列積は列ごとに独立でした(第1巻4章)から、結果の**第 $i$ 列ブロック**(第 $64i$ 〜 $64(i+1)-1$ 列)は $X W_i^Q$ と完全に一致します。「8回の射影」は「1回の行列積と列の切り分け」に置き換わります。$K, V$ も同様です。実装でパラメータを `(512, 512)` の4枚($W^Q, W^K, W^V, W^O$)だけ持てばよいのは、この貼り合わせのおかげです。

**鍵その2: 切り分けた8冊に attention を一斉に適用できます。** `(n, 512)` を8冊の `(n, 64)` ——第1巻6.4の**行列の束** `(8, n, 64)` ——に切り分けます。NumPy では2段階の整形です。

**1段目は reshape です。** `(n, 512) → (n, 8, 64)`。各行の512個を「64個ずつ、8グループ」と区切り直すだけで、数の並び順は変わりません。軸の意味は(単語, head, 次元)になります。

**2段目は transpose(軸の入れ替え)です。** `(n, 8, 64) → (8, n, 64)`。1軸目と2軸目を入れ替えて(head, 単語, 次元)にします。「最後の2軸が行列、残りは束ね方」という第1巻6.4の読み方に当てはめれば、`(n, 64)` の行列が8冊——head $i$ の $Q$ 行列が $i$ 冊目に入った束の完成です。
...
単語aの前半と後半が2単語のように同居し、単語bは1冊目から消えます。shape は `(2, 2, 2)` で同じなのでエラーは出ません。**黙って間違う**のが怖いところです。

reshape にできるのは「並び順を保ったまま区切り直す」ことだけで、軸の**意味の順番**を入れ替えるのは transpose の仕事です。だから「reshape で head の軸を作り、transpose で前に出す」2段が要ります。逆向き(束→1枚)は transpose してから reshape——行きを逆順にたどります。これが論文の Concat の実装です。
...
入口と出口だけ見れば `(n, 512) → (n, 512)` の、ただの sub-layer です。内部でだけ一時的に束へ姿を変える——序章0.3で予告した「multi-head の内部だけ4階に整形する」の中身です。

最後に batch 軸を戻します。文が $B$ 本なら入力は `(B, n, 512)` です。整形を「前から何軸目」ではなく**後ろから数えて**書いておけば(コードでは `swapaxes(-3, -2)` のように負の軸番号を使う)、各 shape の先頭に $B$ が付くだけで同じコードが動きます。束は `(B, 8, n, 64)`——第1巻6.4が「multi-head attention を実装するとき、4次元のテンソルが現れます」と予告した、まさにその4階テンソルです。6巻前の伏線、回収完了です。
...
全文と動作確認は `code/ch04/multi_head.py` です(`python3` で全 assert 通過。`W_q, W_k, W_v, W_o` は h 個の射影を横に並べた `(d_model, d_model)`、第3章の `ch03/attention.py` を import)。本体は空行を除けば**6行**——射影3本、束への分割(同じ行で `split_heads`)、attention 1回、結合と出力射影です。`split_heads` と `combine_heads` も「reshape して swapaxes」「swapaxes して reshape」で、4.3の2段とその逆順そのままです。

mask は引数で受け取り attention にそのまま渡します。`(n, m)` の mask は束のスコア `(h, n, m)` へブロードキャストされ、**全 head に同じ禁止**が掛かります。見る場所は head ごとに違ってよいが、見てはいけない場所は全 head 共通——causal mask の用途(次章)を考えればそうあるべきです。返り値には出力と一緒に attention 重み `(h, n, m)` も返します。head ごとの重みは観察(章末演習)と次章以降のテストに使います。

テストは `code/ch04/test_multi_head.py` に8本です(`python3` で全 assert 通過)。検査項目は次のとおりです。
...
$h = 1$ なら「1等分」、つまり何も分割しません。$d_k = d_{model}$ のフル次元で attention が1回走るだけなので、結果は「射影してから前章の `attention` を1回呼び、$W^O$ を1枚かませたもの」と完全に一致します。multi-head は前章を置き換える別物ではなく、前章を $h = 1$ という特殊な場合として含む**一般化**である——2つの章が地続きであることの、いちばん直接的な証明です。
...
ループ版は論文の2行を上から下へそのまま書いたものです。$W_i^Q$ は $W^Q$ の列ブロック、$\mathrm{head}_i$ は単独 attention の呼び出し、$\mathrm{Concat}$ は `np.concatenate`、最後に $W^O$ です。これと reshape / swapaxes で束ねた `multi_head_attention` の結果が `np.allclose` で一致します。**整形は速さのための実装上の工夫であって、計算の中身は論文の式と寸分違わない**——「式番号とコードの行が1対1対応する」というこの巻のゴールが、assert の形で確認されます。
...
attention を8本に増やしたのにコストは同程度——式で検算します。コストの数え方は第1巻4章と同じ**乗算の回数**です。query 側の系列長を $n$、key / value 側を $m$ とし、`(a, b) @ (b, c)` の乗算回数は $a \cdot b \cdot c$ 回と数えます。

**比較対象その1: フル次元の単独 attention。** $d_{model}$ 次元のまま $Q, K, V$ を作り($W^Q$ は `(512, 512)`)、式(1)を1回実行します。
...
**比較対象その2: $h = 8$ に分割した multi-head。** head 1本の射影は `(n, 512) @ (512, 64)` で $n \cdot d_{model} \cdot d_k$ 回です。役割3つ × head 8本で

$$3h \cdot n\, d_{model}\, d_k = 3\, n\, d_{model} \cdot (h\, d_k) = 3\, n\, d_{model}^2$$

最後の等号で効いたのが**分割の定義 $h \cdot d_k = d_{model}$** です。射影のコストはフル次元1本と8分割で**完全に同じ**。スコアも head 1本 $n \cdot m \cdot d_k$、8本で $n m \cdot (h d_k) = n m\, d_{model}$——同じ。重み付き和も同じ。つまり

$$\text{head 1本のコスト} \approx \frac{\text{フル次元のコスト}}{h}$$

を $h$ 本足すから総額が変わりません。512次元の仕事を64次元 × 8人で分担しただけで、総量は増えていない——"due to the reduced dimension of each head" の中身はこの割り算です。

「同程度(similar)」であって「同じ」ではない理由も挙げます。multi-head には出力射影 $W^O$ の $n\, d_{model}^2$ 回が追加で乗ります。射影3本に対し1本の上乗せで、定数倍は $3 \to 4$、ざっくり3割増です。スコア計算と合わせた全体ではそれより小さな差です。論文の "similar" は正確な筆致でした。

この検算が示すのは、**8つの視点はほぼタダで手に入っている**ことです。では何も失っていないのでしょうか。失っているものはあります——各 head の内積は64次元で取られるので、1本あたりの類似度の「解像度」は512次元より粗いのです。「1つの精密な見方」と「8つの粗い見方」の交換です。どちらが得かは理屈では決まらず、原文の "we found it beneficial"(有益であることを**見出した**)は、実験で確かめた上での選択という宣言として読めます。

前章とのつながりをもう1つ挙げます。式(1)の $\sqrt{d_k}$ の $d_k$ は、multi-head では**head 1本ぶんの次元 64** です。スコアを割るのは $\sqrt{512}$ ではなく $\sqrt{64} = 8$ です。第4巻7章の「内積の分散は次元に比例する」は head の中の64次元の内積に適用されます(実装では `attention` が `Q.shape[-1]` から自動で読み取る)。
