...
その次の Section 4 は、新しい部品も数式もほぼ出てきません。あるのは **Table 1 という1枚の表**と、それをめぐる数段落だけです。タイトルは "Why Self-Attention"。ここは設計の説明ではなく、再帰も畳み込みも捨てて attention だけにした判断の根拠を数字で示す、**設計の弁明**です。2017年にこの弁明を向けられたのは査読者でしたが、いま読む資格を最も持つのは、第6巻で RNN を実装し訓練して**痛い目に遭った**あなたです。系列が長いと訓練が遅くなる、遠くの情報が薄れて消える——あの体感を持つ読者にとって、Section 4 は弁明ではなく**答え合わせ**です。
...
**指標2: 逐次操作数(sequential operations)。** 原文の定義は「並列化できる計算の量を、**必要な逐次操作の最小数**で測る」。どれだけ計算機を並べても順番待ちを強いられるステップが何回あるか、です。互いに独立な計算は手分けで一斉に終わらせられますが、「前の結果がないと次が始められない」計算は計算機を1万台並べても1列で待つしかありません。第6巻5.3で実測した**痛み1(並列化できない)の物差し**がこの列です。
...
$O(n^2)$ と書いたら「$n$ を大きくしたとき、量が $n^2$ のペースで増える」という意味です。注目するのは**増え方だけ**で、定数倍や増え方の遅い項は捨てます。演算回数が $2n^2 d + n^2$ 回なら、係数の2も $n^2$ も捨てて $O(n^2 \cdot d)$。第1巻4章のベンチマークで見たとおり、同じ行列積でも3重ループと `@` では実行時間が数百倍違いました。定数倍は実装次第でいくらでも動くので、層の**方式**を比較する土俵では、実装に左右されない「増え方」だけを見るのです。裏を返せば、$O$ が同じでも実際の速さが同じとは限りません。
...
$n$ は系列の長さ(トークン数)、$d$ は各位置のベクトルの次元($d_{model}$ に相当)です。論文の base model の値で感覚を持っておきましょう——$d = 512$、$n$ は翻訳タスクの文なら数十から百程度。**当時は $n < d$ が普通だった**。この事実が、あとで効いてきます。
...
合わせて約 $2n^2d + n^2$、つまり $O(n^2 \cdot d)$。表のマスと一致しました。第4章の multi-head では1ヘッドあたり $d_k = d/h$ に減りますが、$h$ ヘッド分の合計はほぼ $d$ に戻るのでした(第4章4.5)。表の $d$ はその合計で読みます。
...
根拠は、上の3行が全部行列演算だということです。$QK^T$ は内積の総当たり表で(第1巻4章)、$n^2$ 個のマスはどれも他のマスの結果を待ちません。softmax も行ごとに独立、`weights @ V` も同じ。self-attention は系列を1歩ずつ歩かず、**何歩あってもひと跨ぎ**です。$n$ をいくら伸ばしても順番待ちのステップ数は増えない——これが $O(1)$ の意味です。
...
**計算量 $O(n \cdot d^2)$。** 第6巻5章の RNN の1ステップは $\mathbf{h}_t = f(W_h \mathbf{h}_{t-1} + W_x \mathbf{x}_t)$。主役は $W_h$ `(d, d)` と $\mathbf{h}_{t-1}$ `(d,)` の行列ベクトル積で掛け算 $d^2$ 回。これを $n$ ステップ繰り返すので合計 $O(n \cdot d^2)$ です。
...
つまり Table 1 は新しい主張の表ではなく、**第6巻であなたが踏んだ痛みを3指標に符号化した表**です。そして Self-Attention 行はその2マスがどちらも $O(1)$。待たない、遠くない。第6巻7.4で立てた「attention が本体で、RNN は足枷では?」への定量的回答です(第6巻6.3の痛み3=固定長ボトルネックが表にないのは、attention の導入自体が解決済みだから——第6巻7章)。

### ただし「計算量」の列には注意書きがある

計算量の列だけは様子が違います。Self-Attention の $O(n^2 \cdot d)$ と Recurrent の $O(n \cdot d^2)$、比を取ると $\frac{n^2 d}{n d^2} = \frac{n}{d}$。**大小は $n$ と $d$ の関係次第**です。論文も勝利宣言に条件を付けています。
...
8.1 の感覚値で、BPE トークン化した文(第6巻2章)なら $n$ は数十から百、$d = 512$。たしかに $n < d$ で、当時の翻訳タスクでは計算量でも self-attention に分がありました。しかしこれは**条件付きの勝利**で、$n$ が $d$ を超えた途端に攻守は逆転します。意味は 8.3 で回収します。

### 残り2行は、足早に

Convolutional 行と Self-Attention (restricted) 行は本シリーズが通らなかった道です(畳み込みを深追いしないのは第1章1.3の最短測地線の方針どおり)。表を読むのに必要な分だけ確認します。カーネル幅 $k$ の畳み込み層は各位置の**近傍 $k$ 個だけ**を結びつけるので、1層では遠い位置どうしが繋がらず層を重ねる必要があり、最大経路長は(dilated convolution で)$O(\log_k n)$。計算量は $O(k \cdot n \cdot d^2)$ で再帰層の $k$ 倍。最終行の restricted self-attention は「各位置が近傍 $r$ 個の key だけを見る」変種で、計算量は $O(r \cdot n \cdot d)$ に下がる代わりに経路長が $O(n/r)$ に伸びます。**計算量を削ると経路が伸びる**——このトレードオフは 8.3 への伏線です。
...
机上の検算は済みました。第1巻からの流儀で実測で締めます。検証したいのは Self-Attention 行の計算量 $O(n^2 \cdot d)$。$d$ を固定して $n$ を倍々に増やしたとき、$n^2$ の項が支配的なら実行時間は**1段ごとに約4倍**になるはずです。測る対象は、第3章で自分が書いた `attention` そのものです。
...
全文と動作確認は `code/ch08/attention_scaling.py`(`python3` で全 assert 通過)。各 $n$ で5回測って**最小値**を採るのは、たまたま OS が裏で忙しかった回に引きずられないため。assert を「単調増加 + 線形超え」という緩い条件にしているのは、実行時間が環境(BLAS、CPU、同時に動くプロセス)に強く依存し、「ぴったり4倍」を固定すると正しい実装でも環境差で落ちるためです。固定したいのは係数ではなく、**自乗で効くという傾向**です。
...
「直前との比」を縦に読んでください。3.55、4.35、4.34、3.80——$n$ を2倍にするたびに時間はおよそ**4倍**。log-log の傾き 2.02 は「実行時間は $n$ のほぼ2乗に比例して伸びている」という意味です。Table 1 の左上のマス $O(n^2 \cdot d)$ が、自分のコードの実測として目の前に現れました。検算、完了です。

## 8.3 n² 問題: 系列が長いと自乗で効く

いま実測した数字を、逆側から眺め直します。

直前との比「約4倍」は、8.2 までは勝利の確認でした。しかし同じ数字は**請求書**としても読めます。系列長を2倍にすると計算時間は4倍、10倍なら100倍。しかも attention の重み行列は `(n, n)` なので、**メモリも自乗で**膨らみます。実測では $n = 4096$ で54ミリ秒——たった1層・1ヘッド・順方向だけでこれです。2017年の翻訳タスクでは $n$ は数十から百で、$n < d$ に守られて無視できる額でした。しかし「文」が「文書」になり、書籍1冊やコードベース全体を文脈に入れたくなった現代では $n$ は数十万を超え、この $n^2$ こそが Transformer の最大の請求書になっています。
