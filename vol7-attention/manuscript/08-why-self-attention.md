...
![図8.1](../figures/ch08-attention-timing.png)

図8.1: 上の実測値を両対数(log-log)でプロットしたもの。実測(青)は傾き2の参照線($\propto n^2$、破線)にほぼ重なり、傾き1の線($\propto n$、点線)から一貫して離れていく——「自乗で効く」傾向が1本の直線として見える。(描画コードは `figures/generate_figures.py` 参照)
