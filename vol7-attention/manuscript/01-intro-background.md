...
序章で装備の点検を終えました。1〜6巻で揃えた道具はすべて手元にあり、論文読解マップで「まだ」の印が残っているのは、Section 3.2 の組み立てと Section 3.5 だけ。準備は整っています。いよいよ、論文の1ページ目を開きます。

ただし先に断っておくと、この章には実装がありません。序章0.2で決めた精読の作法は「原文 → 逐行読解 → 単体実装 → テスト → 対応表」でしたが、Abstract・Introduction・Background には、実装すべき部品がまだ登場しないからです。作法がフル稼働するのは第2章からです。

では部品が出てこないのに、なぜ1章を割くのでしょうか。第6巻の終章で、私たちは Introduction と Background を全文読めるようになったはずです。しかしあのときの読みは「自分の踏んだ痛みの答え合わせ」でした。今回は違います。この巻で私たちは Section 3 の全部品を実装していくわけですが、その**設計意図のすべてが、実はこの最初の2ページに予告されています**。何を捨てたのか、なぜ捨てられたのか、捨てた代償をどこで払うのか。それを先に読み取っておくと、第2章以降の各部品が「天下り」ではなく「予告の回収」として読めるようになります。

つまりこの章の仕事は、地図づくりです。最初の2ページの各文に「この巻の第何章で実装するか」という行き先を書き込んでいきます。

## 1.1 Abstract 逐行: "dispensing with recurrence and convolutions entirely" — 第6巻の問いへの宣戦布告として読む

意外に思われるかもしれませんが、Abstract をきちんと逐行で読むのは、シリーズを通してこれが初めてです。第6巻のラスボスは Introduction と Background の文でしたから、Abstract はずっと素通りしてきました。論文の顔である7つの文を、1文ずつ読んでいきます。
...
第1文は現状報告です。「系列変換(sequence transduction)」は、系列を入れて系列を出す問題——第6巻6章で立てた翻訳の問題設定そのものです。「エンコーダとデコーダを含む」は、入力を読む係と出力を書く係の分業(第6巻6.1)。「再帰型(recurrent)」は、隠れ状態を持ち回って1トークンずつ読む RNN(第6巻5章)。ここまで、すべて自分の手で実装したものの名前です。

1つだけ、踏んでいないものがあります。「畳み込み型(convolutional)」です。これは私たちが意図的に通らなかった道で、扱いは 1.3 で決めます。

もう1つ、さりげない単語に注目してください。"complex"——複雑な。論文は冒頭から、既存モデルを「複雑」と形容しています。この形容詞は、2文あとに仕掛けられた対比の前フリです。
...
第2文は、第6巻7章で実装した attention 付き seq2seq の話です。固定長ボトルネック(第6巻6.3)に苦しむ encoder-decoder に attention を付け足すと、長い入力に強くなった——あの体験が、2017年時点の「最高性能のモデル」の標準装備だったと言っています。ここまでの2文で、論文は私たちが第6巻で登った階段をそのままなぞっています。
...
第6巻7.4で、私たちはこういう問いに行き着きました。attention 付き seq2seq の中で、一番遅くて一番弱いのは RNN の部分だった。**「attention が本体で、RNN は足枷では?」**——あれは、実装を終えた私たちの素朴な疑いでした。

この第3文は、その問いへの宣戦布告です。"dispensing with recurrence and convolutions **entirely**"——完全に排除する。「RNN を改良する」でも「RNN への依存を減らす」でもありません。第6巻で私たちが3つの痛み(並列化不能・長距離依存・固定長ボトルネック)の発生源として特定した再帰を、まるごと捨てると宣言しています。第1文の "complex" に対して、ここでは "simple" ——複雑な既存モデルに対し、部品を捨てたからこそ単純になった、という対比もきれいに決まっています。

ただし、威勢のいい宣言には必ず請求書が付きます。第6巻終章で私たちは、RNN を本当に捨てたら困ることを2つ挙げました。1トークンずつ読む仕組みを捨てたら、**語順の情報はどこへ行くのか**。そして encoder-decoder 間ではなく**自分自身への attention とは何なのか**。この請求書の支払いが、それぞれこの巻の第7章(Positional Encoding)と第5章(self-attention)です。Abstract はまだ何も答えていません。宣言だけして、先へ進みます。
...
第4文の売り文句は3つ——品質・並列性・訓練時間。注目すべきは後ろの2つです。"more parallelizable" は、第6巻5.3で訓練時間として体感した痛み1(並列化不能)への返答です。$h_t$ が $h_{t-1}$ を待つ仕組みを捨てたのだから、待ち行列が消える。並列化できれば訓練も速い。3つの売り文句のうち2つが「速さ」の話だというのは、この論文の動機が品質だけではなかったことの証拠です。
...
第5文・第6文は実験結果の数字です。ここは正直に保留します。BLEU という評価指標が何をどう測るのか、28.4 という数がどれくらい偉いのか——**評価指標の詳細は第8巻**で扱います。いま読み取るべきは数字の中身ではなく、文の構造です。「品質で上回った(2 BLEU 以上の改善)」と「訓練コストはごく一部(8 GPU で3.5日)」が、第4文の売り文句の裏付けとして並んでいる。捨てたのに勝った、しかも安く勝った、という主張です。
...
第7文は「翻訳以外でも動く」という一般性の主張です。構文解析(constituency parsing)の中身には立ち入りません(第8巻で概観だけします)。歴史を知っている私たちには、この控えめな一文がむしろ感慨深く読めます。「他のタスクにもよく一般化する」——2017年の著者たちの想像を、その後の現実(GPT も BERT も、あなたが今日会話した AI も)は遥かに超えていきました。

以上7文。まとめると、Abstract の構造はこうです。**現状(1〜2文)→ 宣言(3文)→ 売り文句(4文)→ 証拠(5〜6文)→ 一般性(7文)**。そして3文目の宣言は、第6巻終章の問いから見れば宣戦布告であり、同時に2枚の請求書(語順・self-attention)を切った瞬間でもありました。

## 1.2 Introduction / Background: 第6巻の体感と突き合わせる高速再読

Introduction と Background は、第6巻終章で一度全文読んでいます。だからここでは全文をなぞり直すことはせず、**鍵になる文だけを引いて、第6巻のどの体感と対応するかを固定する**高速再読をします。初読の解説が必要な人は、第6巻終章8.1に戻ってください。
...
第6巻の巻頭では、"This" が何を指すのか分かりませんでした。いまは指せます。1つ目の文の「位置に沿って計算を分解する」、つまり $h_t$ を $h_{t-1}$ から順に作る、あの forループです(第6巻5.1)。系列方向のループは展開できない——各ステップが前のステップの結果を待つからです。第1巻4章で行列積が並列計算と相性抜群なことを実測した私たちにとって、「並列化を不可能にする」がどれほど致命的な悪口かは、訓練の待ち時間(第6巻5.3)が教えてくれた通りです。**痛み1、確認済み。**
...
"without regard to their distance"——距離に関係なく。第6巻7章で attention 付き seq2seq を実装したとき、decoder は入力の全位置を**等しく1ステップで**見渡せました。RNN の記憶のように、遠い情報が薄まることがない(痛み2への返答)。そして文全体を1本のベクトルに圧縮する必要もない(痛み3への返答)。固定長ボトルネックは、実はこの論文の前に attention がすでに解いていた——論文はそれを前提に書かれています。
...
"however" ——この逆接が、論文全体の蝶番です。距離に関係なく依存関係を結べる attention が、よりによって、逐次性という病気を持つ RNN に同居している。第6巻7.4の私たちの違和感(「attention が本体で、RNN は足枷では?」)と、一字一句同じ違和感を、著者たちはここで表明しています。だから提案はこうなります。
...
Background に進みます。Background の前半は畳み込み系の関連研究で、これは 1.3 に回します。ここで拾うべきは、この巻全体の主役の初登場シーンです。
...
第6巻7章の attention は、decoder から encoder へ——**2つの**系列の間に架かる橋でした。ここで定義されている self-attention は "a single sequence"、**単一の**系列の中で位置どうしを関係づけると言っています。文が文自身を見るとはどういうことか。第6巻終章の問いその2が、ここで論文の用語を得ました。答え(Q・K・V がすべて自分自身になる、という種明かし)は第5章で実装とともに確認します。
...
先に宣言します。**この3つを、私たちは深追いしません。** 畳み込みニューラルネットワークの仕組みは、このシリーズが第1巻の序文から一貫して通ってきた最短測地線の上にないからです。Transformer は畳み込みを使いません(Abstract 第3文で "dispensing with ... convolutions entirely" と宣言済みです)。勝者が捨てた部品の内部構造を学ぶことは、論文を読むためにも、実装するためにも、必要ではありません。

ただし、固有名詞は読み飛ばしても、**この段落の論理の骨格**だけは持ち帰る必要があります。論文が自分の強みを主張する、次の対比だからです。
...
骨格を取り出すと、こうなります。畳み込みは「窓」で近所だけをまとめて見る演算です。1層では窓の幅ぶんの距離しか結べないので、離れた2つの位置を関係づけるには層を重ねて信号をバケツリレーする必要があり、必要な段数は距離とともに伸びます——素朴に重ねれば距離に比例して(線形)、窓を飛び飛びに広げる工夫をしても距離の対数で(これが ByteNet 側の意味ですが、工夫の中身は深追いしません)。つまり、RNN の「1トークンずつ」よりはるかにマシでも、**遠い2点を結ぶコストが距離の関数として伸びる**ことに変わりはないのです。
...
これが対比の決着です。self-attention では、どの2つの位置も**1回の attention で直接つながります**。第6巻7章の可視化を思い出してください。attention の重み行列は全位置ペアの類似度を一度に計算していました。隣の単語も、100語先の単語も、同じ1ステップ——距離 $n$ に対して操作数 $O(1)$ です。距離に応じて伸びる($O(n)$ や $O(\log n)$)か、距離によらず一定($O(1)$)か。**この対比の骨格こそ、固有名詞の森から持ち帰るべき唯一の戦利品です。**

そして、この文には請求書も同封されています。"albeit"(〜という代償はあるが)以下です。全位置を重み付き平均でまとめると、情報が混ざってぼやける——その対策が Multi-Head Attention だと、論文はすでにここで予告しています。この予告の回収が、本巻の第4章です。また、この $O(1)$ という主張は Section 4 の Table 1 で計算量の比較表として数字になり、私たちは第8章でそれを検算します。Background の1段落に、この巻の後半の予告が2つも埋まっていたわけです。

なお、ここで安くなったものと、まだ安くなっていないものを区別しておきます。$O(1)$ になったのは「遠い2点を**結ぶ**コスト」であって、計算の総量ではありません。全位置ペアを一度に見るということは、ペアの数だけ計算するということでもあります。この代金の話も第8章で精算します。
