...
Tensor 版 forward を書くには、attention の計算を第5巻の `Tensor` で表現できる必要があります。ところが `tensor_autograd.py` の演算は、加減乗・2次元の行列積・relu・exp・log・sum だけ。転置がなく、multi-head の「頭に裂く」列の切り出しも、Concat も、softmax も、layer norm もありません。

第5巻のファイルも変更禁止です(同じ理由——テスト済みの autograd 本体に手を入れない)。足りない演算は、この章のファイルで**外から**補います。第5巻4章でやったとおり、forward を計算して出力ノードを作り、「親への勾配の配り方」を `_backward` に1つ書く。それだけです。第7巻の `embedding.py` がすでに同じ手口を使っていました(`np.add.at` の backward を `Tensor` の外から差していた)。今回はそれを5回繰り返します。
...
**(4) で、Transformer が初めて学習しました。** このシリーズで組んだ最大のモデル(といっても4万パラメータですが)が、loss 3.01——ほぼ当てずっぽう($\ln 12 \approx 2.48$ よりやや上)——から 0.001 まで降りて、2系列を完全に暗記しました。使った訓練ループは forward → loss → backward → update。第3巻4章で線形回帰に使った**4拍子と、1拍も違いません**。モデルがどれだけ育っても学習の骨格は変わらない——シリーズを貫いてきたこの主張の、これが最終確認です。
...
論文の訓練は「1ステップで約25000ターゲットトークンを処理し、それを10万回」です。私たちの自作スタックは、1ステップで何トークン処理でき、何秒かかるのか。**base model と同じ構成(N=6, d_model=512, d_ff=2048, h=8, 語彙37000)を実際に組んで**測ります。
...
まず認めるべきことを認めます。63M パラメータの本物サイズの Transformer を、自作スタックは**ちゃんと持ち上げました**。組めて、forward が通り、backward まで回る。1ステップ 0.11 秒は、第5巻5.3でスカラー `Value` の遅さに絶望した身からすれば健闘です(行列演算の中身は NumPy、つまり最適化された BLAS が走っているからです)。
