...
前章で encoder-decoder という分業を手に入れ、文字列反転を解き、3つめの痛み——**固定長ボトルネック**に出会いました。encoder は入力文全体を1本の隠れ状態ベクトルに詰め込み、decoder はその1本だけを頼りに出力を書く。入力が長くなるほど1本に収まりきらず、精度は崩れていきました。

これで手元の不満は3つ揃いました。RNN は並列化できない(痛み1)、遠くの情報が薄まる(痛み2)、文全体を1本に圧縮する無理(痛み3)。この章では痛み3を——そして実は痛み2も——一気に解消する **attention(注意機構)** を実装します。論文のタイトルに掲げられた、あの attention です。章の終わりに、あなたは "Attention Is All You Need" というタイトルが何を主張しているのかを、論文を1ページも読まずに自分の実験結果から言い当てることになります。

## 7.1 発想の転換: 1本に圧縮せず、decoder の各ステップで入力の全位置を見直す

第6章の encoder は、入力の各位置で隠れ状態 $\mathbf{h}_1, \mathbf{h}_2, \ldots, \mathbf{h}_L$ を順に計算していました。つまり**位置ごとの読み取り結果は、すでに全部手元にある**。それなのに私たちは最後の1本 $\mathbf{h}_L$ だけを decoder に渡し、残り $L-1$ 本を捨てていた。ボトルネックは能力不足というより、**設計による自業自得**です。

人間の翻訳者にたとえれば、第6章のモデルは原文を一度だけ通読し、原文を伏せて記憶だけで訳す翻訳者です。普通の翻訳者は訳文を1語書くたびに**原文の該当箇所に目を戻します**。原文は机の上にあるのですから、見ればいい。
...
「全部見ればいい」。これが attention の発想のすべてです。圧縮という難題を解いたのではなく、圧縮そのものをやめた。

ただし、decoder のあるステップで「いま必要な位置」とはどこか。文字列反転なら3文字目を書くとき見るべきは入力の後ろから3番目ですが、それはタスクを知っている私たちの答えで、モデルは自分で見つけなければなりません。しかも「3文字目のときは位置 $L-3$」のような if 文をハードコードはできない。**どこを見るかも学習で獲得できる形**——パラメータ付きの微分可能な計算——にする必要があります。ここで、シリーズの最初から温めてきた道具が出番を迎えます。
...
これで各位置に「関連の強さ」の点数 $e_1, \ldots, e_L$ が付きました。「一番点数の高い位置だけを見る」(argmax)としたいところですが、argmax は微分できず勾配が流れません。学習で獲得させるには、**全位置を少しずつ、点数に応じた配合で見る**方が都合がよい。点数の列を「合計1の配合比」に変える関数——第4巻6章で作った **softmax** が、ここで合流します。
...
query で問い、key と照合し、value を取り出す。辞書の引き方そのものです。ただし完全一致した1項目だけを返すのではなく、**全項目を「一致度」の重みで混ぜて返す**。微分可能にするための、たったそれだけの変更です。
...
第6章の `make_data`(文字列反転、長さ可変、seed 42)をそのまま使います。第6章のコードが `code/ch06/` にあればそれを import し、なければ同一仕様の `_make_data_local` に切り替える保険を入れてあります(どちらが使われたかは実行時に表示)。`Tensor` は第5巻5章の自作 autograd をそのまま import します(vol5 側は変更しない)。語彙は8文字、`<bos>` は decoder 側だけが使う開始記号です。全文は `code/ch07/attention_seq2seq.py`(`python3` で全 assert 通過)を参照。

### (2) Tensor に足りない演算を補う

本章のモデルは第5巻5章の `Tensor`(自作 autograd)で組みますが、attention に必要な演算がいくつか足りません。**第5巻のファイルには手を入れず**、足りない分をこの章のファイルで補います。第5巻4章と同じ作業——forward を書き、局所勾配を `_backward` に書く——の繰り返しです。補うのは `tanh_t` / `row_sum`(行ごとの合計で内積を取る)/ `concat_cols` / `col`(列の取り出し)/ `softmax_rows` の5つ。新顔は `softmax_rows` だけで、これも forward は第4巻6.2節の最大値シフト付き softmax、backward は第4巻6章の勾配導出そのままです。
...
**この章で新しい数学は1つも増えていません**——道具はすべて過去の巻からの持ち込みです。5つの全文は `code/ch07/attention_seq2seq.py`(`python3` で全 assert 通過)を参照。
...
短い入力では両者ともほぼ満点。差が出るのは長い入力で、ボトルネック型は 0.508——12文字の半分を間違える水準まで崩れます(第6章で観察した痛み3の再現)。attention 付きは 0.992。**崩れません**。入力が長くなっても decoder は毎ステップ原文の必要な位置を見に行けるので、「覚えきれない」という failure mode 自体が消えたのです。assert はこの差(0.15以上)と attention 側の水準(0.90以上)を固定しています。全文は `code/ch07/attention_seq2seq.py`(`python3` で全 assert 通過)を参照。
...
この章の性能向上は、どの部品の手柄だったか。明らかに attention です。RNN は第6章から1行も変えていないのですから。長い入力で 0.508 → 0.992 という改善は、まるごと attention が持ち込んだものです。

この3部品を「第5章で測った痛み」の観点で採点し直します。

**痛み2(長距離依存)を解いたのは attention。** RNN では位置 $i$ の情報が出力ステップ $t$ に届くまでに、隠れ状態の更新を距離に比例した回数くぐり抜ける必要がありました。遠いほど薄まり、勾配は消えていく(第5章5.4)。attention では、どんなに離れた位置でも $\mathbf{s} \cdot \mathbf{h}_i$ の**内積1発**で直結します。距離という概念そのものが計算経路から消えているのです。

**痛み1(並列化できない)を抱えたままなのは RNN。** encoder は相変わらず $\mathbf{h}_t$ の計算に $\mathbf{h}_{t-1}$ を待ちます。入力が2倍になれば待ち時間も2倍。第1巻4章のベンチマークのとおり、現代のハードウェアは「大きな行列積を一度に」が得意で「小さな計算を順番に」が苦手です。一方 attention は内積・softmax・重み付き和すべて行列演算で、**全位置をまとめて一度に**計算できる形をしています(私たちの実装は教育用に for ループで書きましたが、計算の依存関係としては待ちがありません)。
...
並べてみて、奇妙なことに気づきます。**性能を支えているのは attention で、速度と性能の足を引っ張っているのは RNN です**。私たちは「RNN を attention で補強する」つもりでこの章を書き始めました。しかし正直に評価すると、補強材のほうが本体より強い。むしろこう問うべきに見えてきます——
